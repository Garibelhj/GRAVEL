...
import dgl
import sys
sys.path.append('/home/hongjiegu/projects/GRAVEL/baselines/slotgat')
...
class slotGAT(nn.Module):
    ...
    def l2byslot(self,x):
        ...
        epsilon = self.epsilon.to(x.device)
        x=x / (torch.max(torch.norm(x, dim=2, keepdim=True), epsilon))
        x=x.flatten(1)
        return x

    def forward(self,blocks, features_list,e_feat, get_out="False"):
        with record_function("model_forward"):
            ...
            for l in range(self.num_layers):
                if l == 0:
                    h_block = h[blocks[0].srcdata[dgl.NID]]  # 第一层需要全图ID索引
                else:
                    h_block = h  # 后续层直接用上一层输出
                e_feat_block = e_feat[blocks[l].edata[dgl.EID]]
                h, res_attn = self.gat_layers[l](blocks[l], h_block, e_feat_block,get_out=get_out, res_attn=None) 
                h = h.flatten(1)    #num_nodes*(num_heads*num_ntype*hidden_dim)
                encoded_embeddings=h
            # output projection
            logits, _ = self.gat_layers[-1](blocks[-1], h, e_feat[blocks[-1].edata[dgl.EID]],get_out=get_out, res_attn=None)   #num_nodes*num_heads*num_ntype*hidden_dim
        ...
        epsilon = self.epsilon.to(logits.device)
        logits = logits / (torch.max(torch.norm(logits, dim=1, keepdim=True), epsilon))
