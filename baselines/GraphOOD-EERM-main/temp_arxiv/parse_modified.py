from model_modified import Base, Model_Memory_Efficient
...
def parse_method_ours(args, dataset, device):
    n, c, d = dataset.n, dataset.c, dataset.d
    if args.gnn == 'gcn':
        model = Model_Memory_Efficient(args, n, c, d, 'gcn', device).to(device)
    elif args.gnn == 'sage':
        model = Model_Memory_Efficient(args, n, c, d, 'sage', device).to(device)
    elif args.gnn == 'gat':
        model = Model_Memory_Efficient(args, n, c, d, 'gat', device).to(device)
    elif args.gnn == 'gpr':
        model = Model_Memory_Efficient(args, n, c, d, 'gpr', device).to(device)
    else:
        raise ValueError('Invalid method')
    return model
