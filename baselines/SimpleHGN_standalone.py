...
class SimpleHGN(nn.Module):
    ...
    def forward(self, hg, h_dict, return_hidden=False):
        ...
        return_hidden: bool
            whether to return hidden states before classification layer
            
        Returns
        -------
        dict or tuple
            If return_hidden=False: The embeddings after the output projection.
            If return_hidden=True: (logits_dict, hidden_dict) where hidden_dict contains 
                                 the hidden states before the final classification layer.
        """
        if hasattr(hg, 'ntypes'):
            ...
            with hg.local_scope():
                ...
                # Store hidden states before final layer if requested
                hidden_states = None
                if return_hidden:
                    hidden_states = {}
                ...
                for l in range(self.num_layers):  # noqa E741
                    ...
                    # Store hidden states before the final classification layer
                    if return_hidden and l == self.num_layers - 2:  # Before the last layer
                        hidden_states = to_hetero_feat(h, g.ndata['_TYPE'], hg.ntypes)
            ...
            for i, (layer, block) in enumerate(zip(self.hgn_layers, hg)):
                h = layer(block, h, block.ndata['_TYPE']['_N'], block.edata['_TYPE'], presorted=False)
                
                # Store hidden states before the final classification layer
                if return_hidden and i == len(self.hgn_layers) - 2:  # Before the last layer
                    hidden_states = to_hetero_feat(h, block.ndata['_TYPE']['_N'][:block.num_dst_nodes()], self.ntypes)
            
            h_dict = to_hetero_feat(h, block.ndata['_TYPE']['_N'][:block.num_dst_nodes()], self.ntypes)

        if return_hidden:
            return h_dict, hidden_states
        else:
            return h_dict
