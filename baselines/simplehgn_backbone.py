...
def get_g(benchmark, zero_shot=False):
    ...
    domain['train_mask_zs'] = ~(domain['val_mask'] | domain['test_mask'])
...
def train_epoch_simplehgn_batch(model, optimizer, emb_optimizer, train_loader, embed_layer, node_feats, labels, device, inv_target):
    ...
    for input_nodes, seeds, blocks in train_loader:
        ...
        loss = F.cross_entropy(logits['domain'][valid_label_mask], batch_labels[valid_label_mask])
        if ood_mask.sum() > 0:
            ood_loss_value = ood_loss(logits['domain'][ood_mask], batch_labels[ood_mask])
            loss += ood_loss_value
